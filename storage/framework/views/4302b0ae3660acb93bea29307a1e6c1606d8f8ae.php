...
<div class="container">
  ...
  <lottie-player src="<?php echo e(url('images/lottie/bubble.json')); ?>" class="lottie-bubbles" background="transparent"  speed="1"  style="width: 100px; height: 100px; position:absolute;z-index:1000;margin-left:84%;margin-top: -8%;"  loop  autoplay></lottie-player>
...
<?php if(empty($user->date_verify_email) and ((strpos($user->email,"user") === false) and (strpos($user->email,"stat") === false))): ?>
    <div style="margin-bottom:20px;text-align:center;" class="alert alert-success">
        ...
        Un mail de vérification vous a été envoyé à l'adresse suivante : <strong> <?php echo e($user->email); ?> <strong>.</p>
    </div>
    ...
    <br/>
      <table id="transTable" class="mdl-data-table"  width=100% style="width:100%;z-index: 1100;">
        ...
        <tbody style="background-color:#fff;color:#455469">
          <?php $__currentLoopData = $data_bills; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $value): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <tr>
              <td> <?php echo e($value->deadline); ?></td>
              <td> <span style="color: #4a49b0;font-size: 16px;"><?php echo e($value->title); ?> </span>
                <div> Propriété: <?php echo e($data_infos_housing[$value->customerId][2]); ?></div>
              </td>
              <td><i class="fas fa-user-circle fa-2x reIcon"></i>
                <span style="font-size: 12px;color: #0f541b;"><?php echo e($data_infos_housing[$value->customerId][1]); ?></span>
              </td>
              <td class="reAmount"><span class="green">+</span> <?php echo e($value->amount); ?> FCFA </td>
                <td style="color:#28863e;font-weight: 700;"> <?php echo e($value->status); ?> </td>
                <td> <?php echo e(($value->status == 'paid') ? $value->payment_method  : 'n/a'); ?></td>
            </tr>
...
<?php echo $__env->make('layouts.realEstate', ['notification' => $notification, 'services' => $actived_services, 'profilNotif' => $_SESSION['profilNotif']], array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>